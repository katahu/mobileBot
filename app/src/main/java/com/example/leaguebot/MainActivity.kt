...
class MainActivity : ComponentActivity() {
    ...
    // Читаем файл из assets в String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Держим экран включённым, пока активити видима
        window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)

        // Включаем WebView отладку (для просмотра через chrome://inspect)
        ...
        webView.settings.apply {
            ...
            javaScriptCanOpenWindowsAutomatically = true // Добавлено для поддержки JS диалогов
        }

        // Добавляем WebChromeClient для обработки JavaScript диалогов подтверждения
        webView.webChromeClient = object : WebChromeClient() {
            ...
            override fun onJsConfirm(
                ...
                Log.d("WebView", "onJsConfirm вызван с сообщением: $message")

                AlertDialog.Builder(this@MainActivity)
                    .setMessage(message)
                    .setPositiveButton("OK") { _, _ ->
                        Log.d("WebView", "Пользователь нажал OK")
                        result?.confirm()
                    }
                    .setNegativeButton("Отмена") { _, _ ->
                        Log.d("WebView", "Пользователь нажал Отмена")
                        result?.cancel()
                    }
                    .setOnCancelListener {
                        Log.d("WebView", "Диалог отменен")
                        result?.cancel()
                    }
                    .show()
                return true
    ...
    // Упрощенная инъекция в Shadow DOM
